...
"Kubernetes has built-in Admission Controllers that enforce basic cluster rules like quotas, namespaces, or pod security. But if I need custom logic — like enforcing that only approved images are used — I use an Admission Webhook. I’ve worked with OPA + Gatekeeper, where OPA acts as the decision engine, checking incoming requests against our Rego policies and denying anything that violates our security or operational rules."

API Request ➝ AuthN ➝ AuthZ ➝ Admission Controllers ➝ Admission Webhooks (OPA) ➝ etcd

"Kubernetes includes built-in admission controllers for enforcing core rules like resource quotas, namespace lifecycle, and pod security. If I need to go beyond those — for example, to enforce image whitelisting or labeling standards — I use custom Admission Webhooks. I typically use OPA with Gatekeeper as a policy engine. It evaluates each request against policies written in Rego, and based on that, allows or denies the request before it's persisted to etcd."
